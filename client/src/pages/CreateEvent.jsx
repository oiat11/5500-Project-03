...
export default function CreateEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.message?.includes("Unique constraint failed on the constraint: `Event_name_key`")) {
        const errorMessage = "An event with this name already exists. Please choose a different name.";
  ...
  const recommendDonors = async () => {
    ...
    try {
      ...
      const topDonors = filteredDonors
        .sort((a, b) => (b.total_donation_amount || 0) - (a.total_donation_amount || 0))
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <CardContent>
            ...
            <div className="flex items-end gap-2 my-4">
              <div className="flex-1">
                <Label htmlFor="donorCount">Target Number of Donors to Include</Label>
                <Input 
                  id="donorCount" 
                  type="number" 
                  min="0"
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>Selected Donors ({formData.donors.length}
              {targetDonorCount !== null && targetDonorCount > 0 ? `/${targetDonorCount}` : ''})</CardTitle>
